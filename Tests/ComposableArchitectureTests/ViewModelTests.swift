...
@MainActor
final class ViewModelTests: BaseTCATestCase {
  ...
  func testChangeState() {
    ...
    $viewStore.dynamicallyCall(withKeywordArguments: ["send": ()])
    XCTAssertEqual(emissionCount, 4)
  }
  ...
  func testPublisherFirehose() {
    ...
    let viewStore = ViewModel(store, observe: { $0 })
  ...
  func testEqualityChecks() {
    ...
    let viewStore1 = ViewModel(store1)
    let viewStore2 = ViewModel(store2)
    let viewStore3 = ViewModel(store3)
    let viewStore4 = ViewModel(store4)
  ...
  func testPublisherOwnsViewStore() {
    ...
    var results: [Int] = []
    ViewModel(store, observe: { $0 })
      .publisher
      .sink { results.append($0) }
      .store(in: &self.cancellables)
    
    ViewModel(store, observe: { $0 }).send(())
